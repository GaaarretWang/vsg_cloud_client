...
class vsgRenderer
{
    ...
    vsg::ref_ptr<vsg::WindowTraits> createWindowTraits(string windowTitle, int num, vsg::CommandLine& arguments, vsg::ref_ptr<vsg::Options> options)
    {
        ...
        arguments.read("--screen", windowTraits->screenNum); //设置屏幕编号
        arguments.read("--display", windowTraits->display);  //设置显示器名称
        arguments.read("--samples", windowTraits->samples);
        auto numFrames = arguments.value(-1, "-f"); //获取一个整数值作为帧数
        ...
        if (arguments.read({"--fullscreen", "--fs"})) windowTraits->fullscreen = true;                                           //将窗口配置设置为全屏模式
        if (arguments.read({"--window", "-w"}, windowTraits->width, windowTraits->height)) { windowTraits->fullscreen = false; } //将窗口配置设置为窗口模式，并设置宽度和高度
        if (arguments.read("--IMMEDIATE")) windowTraits->swapchainPreferences.presentMode = VK_PRESENT_MODE_IMMEDIATE_KHR;       //设置不同的交换链偏好和窗口属性
        ...
        {
            windowTraits->swapchainPreferences.presentMode = VK_PRESENT_MODE_IMMEDIATE_KHR;
            windowTraits->width = 96, windowTraits->height = 54;
...
public:
    void initRenderer(int argc, char** argv)
    {
        ...
        //创建窗口数据
        auto cadWindowTraits = createWindowTraits("CADModel", 0, arguments, options);
        ...
        double nearFarRatio = arguments.value<double>(0.0001, "--nf");       //近平面和远平面之间的比例
        auto numShadowMapsPerLight = arguments.value<uint32_t>(10, "--sm"); //每个光源的阴影贴图数量
        auto numLights = arguments.value<uint32_t>(2, "-n");                //光源数量

        //-----------------------------------------设置shader------------------------------------//
        CADMesh cad;
        vsg::ref_ptr<vsg::ShaderSet> phongShader = cad.buildShader("../data/shaders/standard.vert", "../data/shaders/standard_phong.frag");
        ...
        if (numLights >= 1)
        {
            ...
            directionalLight->direction = vsg::normalize(vsg::vec3(1.0, 0.0, -1.0));
    ...
    bool render(std::vector<std::vector<uint8_t>> &color, std::vector<double>& lookAtVector, const std::string& real_color, const std::string& real_depth){
        ...
        if (viewer->advanceToNextFrame() && Env_viewer->advanceToNextFrame() && Shadow_viewer->advanceToNextFrame() && Final_viewer->advanceToNextFrame())
        {
            ...
            auto color_pixels = convertimage->convertColor(real_color);
            auto depth_pixels = convertimage->convertDepth(real_depth);
